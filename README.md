...
基于 [pi](https://pi.dev) 打造的个人 AI 助手。Dito 拥有：
...
```bash
dito voice          # 全屏水波界面，空格键开始说话（或用唤醒词）
```

> 配置 `wakeWord`（如 `hey,你好`）后，待机时本地 whisper 低成本轮询 2.5 秒短音频，听到唤醒词会用蒂特的声音应一声「在的啊」并进入对话；空格键始终可手动唤醒。
...
- **群聊响应**：被唤醒（@机器人 或含唤醒词）**必答**；普通群聊消息按概率回复（默认 0.2，`groupReplyChance` 可调）；`wakeOnlyGroups` 可把指定群设为仅唤醒响应；表情回应与回复绑定，私聊不受限
...
- **iPhone（PWA）**：把移动端仓库 `pwa/` 目录配到 `channels.mobile.pwaDir`（或中继 `--pwa-dir`），扫码后直接在浏览器里和电脑上的 Dito 对话（支持按住说话→语音识别），Safari「添加到主屏幕」即成独立应用
